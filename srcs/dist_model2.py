...
class BackBone(nn.Module):
    ...
    def __init__(self, block, num_block, geom, use_bn=True):
        ...
        # Block 1
        self.conv1 = conv3x3(36, 28)
        self.conv2 = conv3x3(28, 28)
        ...
        # Block 2-5
        self.in_planes = 28
        self.block2 = self._make_layer(block, 20, num_blocks=num_block[0])
        self.block3 = self._make_layer(block, 44, num_blocks=num_block[1])
        self.block4 = self._make_layer(block, 60, num_blocks=num_block[2])
        self.block5 = self._make_layer(block, 92, num_blocks=num_block[3])

        # Lateral layers
        self.latlayer1 = nn.Conv2d(
            368, 192, kernel_size=1, stride=1, padding=0)
        self.latlayer2 = nn.Conv2d(
            240, 124, kernel_size=1, stride=1, padding=0)
        self.latlayer3 = nn.Conv2d(176, 92, kernel_size=1, stride=1, padding=0)

        # Top-down layers
        self.deconv1 = nn.ConvTranspose2d(
            192, 124, kernel_size=3, stride=2, padding=1, output_padding=1)
        p = 0 if geom['label_shape'][1] == 175 else 1
        self.deconv2 = nn.ConvTranspose2d(
            124, 92, kernel_size=3, stride=2, padding=1, output_padding=(1, p))
...
class Header(nn.Module):
    ...
    def __init__(self, use_bn=True):
        ...
        self.use_bn = use_bn
        bias = not use_bn
        self.conv1 = conv3x3(92, 92, bias=bias)
        self.bn1 = nn.BatchNorm2d(92)
        self.conv2 = conv3x3(92, 92, bias=bias)
...
class PIXOR_DIST2(nn.Module):
    ...
    def __init__(self, geom, use_bn=True, decode=False):
        super(PIXOR_DIST2, self).__init__()
