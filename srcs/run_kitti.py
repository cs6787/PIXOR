...
class Detector(object):
    ...
    def __init__(self, config, cdll, name):
        ...
        self.net.load_state_dict(torch.load(
            config['ckpt_name'], map_location=self.device))
        self.net.eval()
    ...
    def preprocess(self, velo, path):
        ...
        velo_processed = torch.from_numpy(
            velo_processed).permute(2, 0, 1).to(self.device)
    ...
    def postprocess(self, pred):
        ...
        # NMS
        selected_ids = non_max_suppression(
            corners, scores, self.config['nms_iou_threshold'])
...
def run(dataset, save_path, name, model, height=400):
    ...
    imshape = (config['geometry']['input_shape'][1] *
               2, config['geometry']['input_shape'][0])
    ...
    for (item, velo_file) in enumerate(dataset.velo_files):
        ...
        print(time)
        merged_im = np.zeros(
            (pred_bev.shape[0], pred_bev.shape[1] * 2, 3), dtype=np.uint8)
        ...
        #print(crop_i, crop_j)
        merged_im[crop_x:crop_y, :pred_bev.shape[1],
                  :] = image[:, crop_i:crop_j, :]
